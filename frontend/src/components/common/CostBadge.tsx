...
export function CostBadge({ usage }: { usage: TokenUsage }) {
  ...
  return (
    <div className="flex items-center gap-3 font-mono text-[14px] tracking-wider">
      <div className="flex items-center gap-1.5 text-deep-200/85">
        <span className="text-amber-300/85">API</span>
        <span className="text-deep-100">{usage.total_api_calls}</span>
      </div>
      <span className="w-px h-3 bg-deep-400/15" />
      <div className="flex items-center gap-1.5 text-deep-200/85">
        <span className="text-amber-300/85">TKN</span>
        <span className="text-deep-100">{totalTokensK}K</span>
      </div>
      <span className="w-px h-3 bg-deep-400/15" />
      <div className={`flex items-center gap-1.5 ${costWarning ? 'text-earth-rust' : 'text-deep-200/85'}`}>
        <span className={costWarning ? 'text-earth-rust/70' : 'text-amber-300/85'}>USD</span>
