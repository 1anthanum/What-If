  ...
  return (
    <div className="glass border border-amber-300/10 rounded-lg p-4 space-y-3">
      <div className="flex items-center gap-2">
        ...
        <span className="text-[10px] font-mono text-amber-300/50 uppercase tracking-wider">
          决策分支树
        </span>
      ...
      <div className="space-y-1 pl-2">
        ...
        <div className="flex items-start gap-2">
          <span className="text-[9px] font-mono text-purple-400/50 mt-0.5 shrink-0 w-4">◈</span>
...
function CycleBranch({
  ...
  return (
    <div className="ml-4 border-l border-deep-400/10 pl-3 space-y-1">
      {/* Chosen path */}
      <div className="flex items-start gap-2">
        <span className="text-[9px] font-mono text-amber-300/40 mt-0.5 shrink-0 w-4">{cycle.cycle}</span>
        <div className="flex-1 min-w-0">
          <p className="text-[10px] text-white/55 leading-relaxed truncate">
            {chosenQuestion || '(收敛)'}
          </p>
        </div>
        {hasBranch && (
          <button
            ...
            className="text-[8px] font-mono text-deep-200/30 hover:text-amber-300/50 transition-colors shrink-0 px-1.5 py-0.5 border border-deep-400/10 rounded"
          >
      ...
      {showAlternates && alternates.map((alt, i) => (
        <div key={i} className="ml-4 flex items-start gap-2 opacity-40">
          <span className="text-[9px] font-mono text-deep-200/30 mt-0.5 shrink-0 w-4">↳</span>
          <p className="text-[10px] text-deep-200/40 leading-relaxed italic">
            {alt}
          </p>
          <span className="text-[7px] font-mono text-deep-200/20 shrink-0 mt-0.5 border border-deep-400/8 rounded px-1">
            未探索
          </span>
        </div>
      ))}
