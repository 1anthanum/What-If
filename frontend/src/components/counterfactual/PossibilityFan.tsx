  ...
  return (
    <div className="space-y-6">
      ...
      <div className="glass border border-amber-300/40 rounded-lg p-4">
        ...
        <div className="text-[14px] text-deep-200/50 bg-deep-700/30 rounded px-3 py-2 border border-deep-400/40">
          假设：「{modification}」
        </div>
      </div>
              ...
              <div key={branch.cluster_id} className="relative">
                ...
                <button
                  ...
                  className={`w-full text-left transition-all duration-300 rounded-lg border ${
                    isSelected
                      ? 'glass border-amber-300/55 shadow-glow-sm'
                      : 'bg-deep-800/40 border-deep-400/40 hover:border-deep-400/45'
                  }`}
                >
                  <div className="p-4">
                    ...
                    <div className="flex items-start justify-between gap-3 mb-2">
                      <div className="flex-1">
                        ...
                        <p className="text-[14px] text-deep-200/85 mt-1 leading-relaxed">
                          {branch.explanation}
                        </p>
                      </div>
                      ...
                      <div className="flex flex-col items-end gap-1 shrink-0">
                        <span
                          className="text-[14px] font-mono font-bold px-2 py-0.5 rounded"
                          style={{
                        ...
                        <div className="mb-2 mt-1">
                          <div className="flex flex-wrap gap-1 mb-1">
                            {coalition.members.map((member, mi) => (
                              <span
                                key={mi}
                                className="text-[14px] bg-amber-300/10 text-amber-300/95 rounded-full px-2 py-0.5 border border-amber-300/45"
                              >
                                {member}
                              </span>
                            ))}
                          ...
                          {coalition.shared_interest && (
                            <p className="text-[15px] text-deep-200/35 italic">
                              共同利益: {coalition.shared_interest}
                            </p>
                          )}
                          {coalition.conflict_points.length > 0 && (
                            <p className="text-[15px] text-earth-rust/40 mt-0.5">
                    ...
                    {branch.key_divergences.length > 0 && (
                      <div className="mt-3 flex flex-wrap gap-1.5">
                        {branch.key_divergences.slice(0, 3).map((d, i) => (
                          <span
                            key={i}
                            className="text-[15px] text-deep-200/85 bg-deep-700/30 rounded px-1.5 py-0.5 border border-deep-400/40"
                          >
                ...
                {isExpanded && branch.timeline_points.length > 0 && (
                  <div className="mt-3 glass border border-amber-300/35 rounded-lg p-5">
                    ...
                    {/* Summary */}
                    {branch.summary && (
                      <div className="mt-4 pt-3 border-t border-deep-400/40">
                        <h5 className="text-[15px] font-mono text-deep-200/85 uppercase tracking-wider mb-1.5">
                          分析摘要
                        </h5>
                        <p className="text-xs text-deep-100/95 leading-relaxed">
                          {branch.summary}
                        </p>
                      </div>
                    )}
          ...
          <div key={`${point.year}-${idx}`} className="relative">
            <div className="flex items-stretch min-h-[56px]">
              ...
              <div className="flex-1 pr-4 flex justify-end">
                <p className="text-[14px] text-deep-200/85 leading-relaxed max-w-[200px] text-right">
                  {point.actual}
                </p>
              </div>
              ...
              <div className="relative flex flex-col items-center z-10 w-16 shrink-0">
                ...
                >
                  ...
                  >
                    <span className="text-[14px] font-mono font-bold text-white/70">
                      {point.year}
                    </span>
                  </div>
                </button>
              ...
              <div className="flex-1 pl-4">
                <p className="text-[14px] text-amber-300/95 leading-relaxed max-w-[200px]">
            ...
            {/* Expanded detail */}
            {isExpanded && (
              <div className="mx-auto max-w-md my-2 p-3 bg-deep-700/30 border border-deep-400/40 rounded-lg">
                <div className="flex items-center gap-2 mb-2">
                  <span
                    className="px-1.5 py-0.5 rounded text-[14px] font-mono uppercase"
                    style={{
                ...
                <p className="text-[14px] text-deep-100/95 leading-relaxed">
                  {point.reasoning}
                </p>
              </div>
            )}
