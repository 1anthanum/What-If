    ...
    <div className="space-y-0">
      ...
      <div className="flex items-center gap-4 mb-6">
        <div className="flex items-center gap-2">
          ...
          <span className="text-[10px] font-mono text-deep-200/50 uppercase tracking-wider">
            实际历史
          </span>
        </div>
        <div className="flex items-center gap-2">
        ...
        {timelinePoints.map((point, idx) => {
          ...
          return (
            <div key={`${point.year}-${idx}`} className="relative group">
                ...
                <div className="relative flex flex-col items-center z-10 w-20 shrink-0">
                  ...
                  >
                    ...
                    {vulnByYear.has(point.year) && (
                      <div
                        className="absolute -left-1 -top-1 w-4 h-4 rounded-full flex items-center justify-center text-[8px]"
                        style={{
                    ...
                    {annotatedYears.has(point.year) && (
                      <div className="absolute -left-1 -bottom-1 w-4 h-4 rounded-full flex items-center justify-center text-[8px] bg-blue-500/50 border border-blue-400/60">
                        ✎
                      </div>
                    )}
                  </button>
              ...
              {isExpanded && (
                <div className="mx-auto max-w-2xl my-3 glass border border-amber-300/10 rounded-lg overflow-hidden">
                  {/* Category bar */}
                  <div
                    className="h-1"
                  ...
                  <div className="p-4 space-y-3">
                    ...
                    {/* Reasoning */}
                    <div>
                      <h4 className="text-[10px] font-mono text-amber-300/40 uppercase tracking-wider mb-1.5">
                        推理过程
                      </h4>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <div className="p-2.5 rounded bg-deep-700/30 border border-deep-400/10">
                        <h5 className="text-[9px] font-mono text-deep-200/40 uppercase mb-1">
                          实际历史
                        </h5>
                        <p className="text-[11px] text-deep-200/60 leading-relaxed">
                          {point.actual}
                        </p>
                      </div>
                      <div className="p-2.5 rounded bg-amber-300/[0.04] border border-amber-300/10">
                        <h5 className="text-[9px] font-mono text-amber-300/50 uppercase mb-1">
                          反事实
                        </h5>
                      ...
                      return (
                        <div className="p-3 rounded bg-red-500/[0.06] border border-red-400/15 space-y-2">
                          <div className="flex items-center justify-between">
                            <h5 className="text-[9px] font-mono text-red-300/70 uppercase tracking-wider">
                              脆弱性分析
                            </h5>
                            <span className="text-[9px] font-mono px-1.5 py-0.5 rounded bg-red-500/15 text-red-300/70">
                              严重度 {Math.round(vuln.severity * 100)}%
                            </span>
                          </div>
                          <div>
                            <p className="text-[10px] text-deep-200/40 mb-0.5">攻击向量</p>
                            <p className="text-[11px] text-red-200/60 leading-relaxed">{vuln.attack_vector}</p>
                          </div>
                          <div>
                            <p className="text-[10px] text-deep-200/40 mb-0.5">反面证据</p>
                            <p className="text-[11px] text-deep-200/60 leading-relaxed">{vuln.counter_evidence}</p>
                          </div>
                          {vuln.alternative_outcome && (
                            <div>
                              <p className="text-[10px] text-deep-200/40 mb-0.5">替代结果</p>
                              <p className="text-[11px] text-deep-200/60 leading-relaxed">{vuln.alternative_outcome}</p>
                            </div>
                          )}
                        </div>
                      );
                    ...
                    {annotations.filter(a => a.year === point.year).map(ann => (
                      <div key={ann.year} className="p-3 rounded bg-blue-500/[0.06] border border-blue-400/15 space-y-1.5">
                        <div className="flex items-center justify-between">
                          <h5 className="text-[9px] font-mono text-blue-300/70 uppercase tracking-wider">
                            用户标注
                          </h5>
                          <span className="text-[9px] font-mono px-1.5 py-0.5 rounded bg-blue-500/15 text-blue-300/60">
                            {CONSTRAINT_LABELS[ann.constraint_type] || ann.constraint_type}
                          </span>
                        </div>
                        <p className="text-[11px] text-blue-200/70 leading-relaxed">{ann.correction}</p>
                        {ann.source_description && (
                          <p className="text-[10px] text-deep-200/30 italic">来源: {ann.source_description}</p>
                        )}
                      </div>
                    ))}
                    ...
                    <button
                      ...
                      className="w-full py-1.5 rounded border border-blue-400/15 bg-blue-500/[0.04] text-[10px] font-mono text-blue-300/50 hover:text-blue-300/80 hover:bg-blue-500/[0.08] transition-colors"
                    >
