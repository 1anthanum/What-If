            ...
            <button
              ...
              className={`text-left rounded-lg p-3 transition-all duration-300 border ${
                isSelected
                  ...
                  : 'border-deep-400/8 bg-deep-700/20 hover:border-deep-400/15 hover:bg-deep-700/30'
              }`}
            >
              <div className="flex items-start gap-3">
                {/* Avatar placeholder */}
                <div className={`w-8 h-8 rounded-full flex items-center justify-center text-[10px] font-mono font-bold border shrink-0 ${
                  isSelected
                    ? 'border-amber-300/30 bg-amber-300/15 text-amber-300/80'
                    : 'border-deep-400/15 bg-deep-600/30 text-deep-200/40'
                }`}>
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between">
                    <h5 className={`text-[11px] font-medium truncate ${
                      isSelected ? 'text-amber-300/90' : 'text-white/70'
                    }`}>
                      {persona.name}
                    </h5>
                    {persona.era && (
                      <span className="text-[8px] font-mono text-deep-200/25 shrink-0 ml-2">
                        {persona.era}
                      </span>
                    )}
                  </div>
                  ...
                  {isSelected && persona.worldview && (
                    <div className="mt-2 pt-2 border-t border-deep-400/8 space-y-1">
                      <p className="text-[9px] text-deep-200/30 leading-relaxed italic">
                        "{persona.worldview}"
                      </p>
                      {persona.known_positions && persona.known_positions.length > 0 && (
                        <div className="flex flex-wrap gap-1 mt-1">
                          {persona.known_positions.slice(0, 3).map((pos, idx) => (
                            <span
                              key={idx}
                              className="text-[8px] bg-deep-600/30 text-deep-200/35 rounded-full px-2 py-0.5 border border-deep-400/8"
                            >
                              {pos}
                            </span>
                          ))}
                ...
                <div className={`w-4 h-4 rounded border flex items-center justify-center shrink-0 mt-0.5 transition-all ${
                  isSelected
                    ...
                    : 'border-deep-400/20 bg-deep-700/30'
                }`}>
                  {isSelected && (
