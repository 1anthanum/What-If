  ...
  if (!graphId && status !== 'generating') {
    return (
      <div className="max-w-xl mx-auto animate-fade-in">
        ...
        {/* Examples */}
        <div className="mb-10">
          <p className="text-[15px] font-mono text-deep-200/70 mb-3 uppercase tracking-[0.2em]">
            预设场景
          </p>
          <div className="grid grid-cols-3 gap-2.5">
            {EXAMPLE_SCENARIOS.map((ex, i) => (
              ...
              >
                ...
                <span className="block text-[14px] text-deep-200/75 mt-1.5 leading-relaxed">
                  {ex.hypothesis}
                </span>
              </button>
            ))}
  ...
  if (status === 'generating') {
    return (
      <div className="max-w-xl mx-auto animate-fade-in text-center py-20">
        <div className="inline-flex items-center justify-center w-16 h-16 rounded-full bg-amber-300/[0.05] border border-amber-300/40 mb-6">
          <span className="w-8 h-8 border-2 border-amber-300/55 border-t-amber-300 rounded-full animate-spin" />
        </div>
        <h3 className="text-lg font-light text-white mb-3">正在构建因果图谱</h3>
        <p className="text-xs text-deep-200/75 mb-6">AI 正在分析场景并建立因果关系网络…</p>

        {streamingText && (
          <div className="glass rounded-lg p-4 text-left max-w-md mx-auto max-h-48 overflow-y-auto">
            <p className="text-[14px] font-mono text-amber-300/75 mb-2">▸ AI OUTPUT</p>
            <p className="text-[15px] text-deep-100/85 leading-relaxed whitespace-pre-wrap">
  ...
  return (
    <div className="animate-fade-in">
      ...
      <div className="flex items-center justify-between mb-5">
        ...
        <button
          onClick={reset}
          className="text-[14px] font-mono text-deep-200 hover:text-amber-300 transition-colors px-2.5 py-1 border border-deep-400/45 rounded hover:border-amber-300/55"
        >
          NEW
        </button>
      </div>
